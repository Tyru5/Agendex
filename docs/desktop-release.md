...
1. **Preflight** resolves release metadata and target platforms, verifies the download page version, and runs checks + desktop tests.
...
prereleases); every asset URL on the page derives from that constant. Commit that change before
creating the release tag; release preflight rejects a stable release whose download page is stale.
Local `bun run release:desktop:mac` / `:win` keep the same download-page update on disk (commit it
