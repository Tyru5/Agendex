...
`packages/ee/convex/workspaceCryptoInventory.ts` classifies every schema field. The `encrypted` category includes references to encrypted storage objects; the reference itself remains visible. `unused_when_encrypted` fields must be cleared or disabled once sealing completes.

| Records or fields                                               | Content protection                                                                                                                                                              | Server-readable metadata                                                                                     |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Plans and versions                                              | Titles, bodies, paths, workspaces, and metadata are encrypted. Plaintext `titleNormalized` is cleared along with `title`; raw identities and unkeyed `contentHash` are removed. | Agent and `agentNormalized`, format, version, timestamps, record IDs, `lowValue`, and keyed equality tokens. |
| Comments, annotations, links, tags, collections, and writebacks | Text, attachment metadata, anchors, names, descriptions, URLs, and feedback are encrypted.                                                                                      | Relations, lifecycle state, timestamps, tag colors, and routing metadata.                                    |
| Attachments and agent avatars                                   | Stored file bytes are encrypted.                                                                                                                                                | Storage references, encrypted sizes, record IDs, and agent names.                                            |
| Daemon heartbeats                                               | Hostnames and IP addresses are encrypted. Cloud usage sync is disabled; `usageSnapshots` and `usageUpdatedAt` are cleared during sealing.                                       | Device ID, process ID, start/heartbeat timestamps, key epoch, and unlock status.                             |
| Account preferences                                             | No workspace content is stored.                                                                                                                                                 | Privacy preferences, view preferences, and completed product-tour version.                                   |
| Account deletion jobs                                           | No content or decrypted keys are needed to delete an account.                                                                                                                   | Record references, deletion phase, retries, operational errors, Stripe cancellation state, and timestamps.   |
| Avatar upload reservations                                      | No file bytes or filenames are stored in the reservation.                                                                                                                       | Owner, agent, creation time, and expiration.                                                                 |
| Share links and access proofs                                   | Sharing is disabled and links are revoked. A prior access proof cannot authorize an encrypted workspace.                                                                        | Any remaining proof contains only its revoked link reference and timestamps.                                 |
| Exports                                                         | Readable exports are assembled on the client. Cloud backups contain ciphertext.                                                                                                 | Archive status, size, generated filename, timestamps, and build lease token/expiry.                          |
| Membership, billing, and encryption settings                    | Private keys and workspace keys are wrapped; invite secrets are encrypted.                                                                                                      | Identity and billing fields, public keys, KDF parameters, grants' routing fields, and operation state.       |
